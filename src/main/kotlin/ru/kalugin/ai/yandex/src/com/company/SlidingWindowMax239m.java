...
public class SlidingWindowMax239m {
    ...
    public static int[] maxSlidingWindow(int[] nums, int k) {
        ...
        var queueP = new PriorityQueue<Integer>(Comparator.reverseOrder());
        var slidingWindox = new int[k];
        while (r < nums.length) {
            queue.offer(nums[r]);
            queueP.offer(nums[r]);
            if (queue.size() > k) {
                queueP.remove(queue.poll());
                result[cntRes] = queueP.peek();
                cntRes++;
            }
            if (r == (k - 1)) {
                result[cntRes] = queueP.peek();
    ...
    public static int[] maxSlidingWindowF(int[] nums, int k) {
        ...
        var cntRes = 0;
        var result = new int[nums.length - k + 1];
        var slidingWindow = new int[k];
        int[] sortedArr = new int[k];
        var forDel = 0;
        //while (r < (nums.length - k +1)) {
        while (r < nums.length) {
            forDel = slidingWindow[r % k];
            slidingWindow[r % k] = nums[r];
            if (r > (k - 1)) {
                result[cntRes] = addNewValueAndRemoveOld(sortedArr, forDel, nums[r]);
                cntRes++;
            }
            if (r == (k - 1)) {
                sortedArr = Arrays.copyOf(slidingWindow, k);
                Arrays.sort(sortedArr);
                result[cntRes] = sortedArr[k - 1];
                cntRes++;
            }
            r++;
        }
    ...
    //private static int addNewValueAndRemoveOld(int[] sortedArr, int valueForDel, int newValue, int[] slidingWindox, int[] result, int pointerTohead) {
    private static int addNewValueAndRemoveOld(int[] sortedArr, int valueForDel, int newValue) {
        var delElemFound = false;
        var sortArrLen = sortedArr.length;
        for (var i = 0; i < sortArrLen; i++) {
            if (sortedArr[i] == valueForDel && !delElemFound) {
                delElemFound = true;
            }
            if (delElemFound && (i != (sortArrLen - 1))) {
                sortedArr[i] = sortedArr[i + 1];
            }
        }

        sortedArr[sortArrLen - 1] = Integer.MIN_VALUE;

        for (var i = sortArrLen - 2; i > -1; i--) {
            if (sortedArr[i] > newValue) {
                sortedArr[i + 1] = sortedArr[i];
            } else {
                sortedArr[i + 1] = newValue;
                break;
            }
            if (i == 0) {
                sortedArr[i] = newValue;
                break;
            }
        }
